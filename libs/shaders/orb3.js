...
float SineEggCarton(vec3 p) {
    return .1 + abs(cos(p.x) + sin(p.y) - sin(p.z)) * 1.2 * orbOpacity;
...
vec3 GetColor(vec3 p) {
    float amount = clamp((1.8 - length(p)) / 2.0, 0.0, 1.0);

    vec3 col = 0.5 + 0.5 * cos(6.28319 * (vec3(0.2, 0.0, 0.0) + amount * vec3(1.0, 0.9, 0.8)));
...
//
//    vec2 point = coord / iResolution.xy - 0.5;
//
//    float t = 0.0;
//    float scale = 2.0;
//
...
//
//    float d = Map(rd, scale);
//
//
//
//
//    vec4 color = vec4(rd, 1.0);
//
//    gl_FragColor = color;
//}
...
void main() {
    ...
    vec3 col = vec3(0.02, 0.04, 0.09) / rz; // colors
    col = pow(abs(col), vec3(1.01)) - (abs((iMouse.x)) * .00005);
    gl_FragColor += vec4(col, 1.0);

    //Normalized point (between 0 and 1)
    vec3 rd = normalize(vec3(2.0 * coord - iResolution.xy, -iResolution.y * 4.0));
    //vec3 rd = normalize(vec3(2.0 * coord - iResolution.xy, -iResolution.y));
    ...
    for (int i = 0; i < 80; i++) {
        ...
        float d = sdOctahedron(p, 0.8, scale);
        //float d = Map(p, scale);
        //float d = Map(p, scale);
        ...
        gl_FragColor.rgb += (0.02 * GetColor(p) * (audio1 * .6)) * orbOpacity;
